    ...
    class Program
    {
        ...
        private static uint AssemblyLine(string line)
        {
            ...
            if (command[0].ToUpper() != "STOP")
            {
                if (command[1].Length > 0)
                {
                    ...
                    {
                        value |= ( ConvertRegisterValue(command[1].Trim()) << 21 ) ;
                    }
                }
            }
            return value;
        ...
        private static uint ConvertMnemonic(string v)
        {
            ...
            switch (instruction[0].ToUpper())
            {
                ...
                case "MULT":
                    value = 0b1110_0000_0000_0000_0000_0000_0000_0000;
                    break;
            ...
            if (instruction[0].ToUpper() != "STOP")
            {
                if (instruction[1].Length > 0)
                {
                    if (uint.TryParse(instruction[1], out uint argument))
                    {
                        value |= argument; 
                    }
                    else
                    {
                        value |= (ConvertRegisterValue(instruction[1].Trim()) << 24 );
                    }
                }
            }
